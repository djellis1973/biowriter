# biographer.py – MemLife main app (with vignettes, custom topics, and session management)
...
def get_user_vignettes(user_id):
    if not VignetteManager:
        return []
    try:
        vignette_manager = VignetteManager(user_id)
        return vignette_manager.get_all_vignettes(include_drafts=True)
    except:
        return []
...
def add_vignette(user_id, topic, content):
    ...
    try:
        vignette_manager = VignetteManager(user_id)
        vignette_manager.create_vignette(
            title=topic,
            content=content,
            theme=topic,
            tags=["vignette"],
...
def add_vignette_to_main_story(user_id, vignette_index, session_id, topic_override=None):
    ...
    if vignette_index >= len(vignettes):
        return False
    
    vignette = vignettes[vignette_index]
    topic = topic_override or f"Vignette: {vignette.get('title', 'Untitled')}"
    content = vignette.get('content', '')
...
def show_vignette_modal():
    ...
    with st.container():
        ...
        # Use the VignetteManager's creator
        vignette_manager = VignetteManager(st.session_state.user_id)
        
        # Define callback for publish
        def on_publish(vignette):
            st.success(f"Vignette '{vignette['title']}' published successfully!")
            st.session_state.show_vignette_modal = False
        
        # Display the vignette creator
        vignette_manager.display_vignette_creator(on_publish=on_publish)
        
        # Add back button
        if st.button("⬅️ Back", key="vignette_back_btn"):
            st.session_state.show_vignette_modal = False
            st.rerun()
...
def show_custom_topic_modal():
    ...
    with st.container():
        ...
        # Use TopicBank to create custom topic
        topic_bank = TopicBank(st.session_state.user_id)
        
        # Display the topic creator
        topic_bank.display_topic_creator()
        
        # Add back button
        if st.button("⬅️ Back", key="custom_topic_back_btn"):
            st.session_state.show_custom_topic_modal = False
            st.rerun()
...
def show_topic_browser():
    ...
    with st.container():
        st.markdown('<div class="topic-browser-modal">', unsafe_allow_html=True)
        ...
        # Create topic bank and display browser
        topic_bank = TopicBank(st.session_state.user_id)
        topic_bank.display_topic_browser(on_topic_select=on_topic_select)
        
        # Add back button
        if st.button("⬅️ Back", key="topic_browser_back_btn"):
            st.session_state.show_topic_browser = False
            st.rerun()
...
def show_session_creator():
    ...
    with st.container():
        st.markdown('<div class="session-creator-modal">', unsafe_allow_html=True)
        st.title("📋 Create Custom Session")
        
        # Create session manager
        session_manager = SessionManager(SESSIONS, st.session_state.user_id)
        
        # Display session creator
        session_manager.display_session_creator()
        
        # Add back button
        if st.button("⬅️ Back", key="session_creator_back_btn"):
            st.session_state.show_session_creator = False
            st.rerun()
...
def show_session_manager():
    """Show the session manager with grid view"""
    if not SessionManager:
        st.error("Session module not available")
    ...
    with st.container():
        st.markdown('<div class="session-manager-modal">', unsafe_allow_html=True)
        st.title("📖 Session Manager")
        
        # Define callback for session selection
        def on_session_select(session_id):
            # Find the session index
            all_sessions = SESSIONS + session_manager.custom_sessions
            for i, session in enumerate(all_sessions):
                if session["id"] == session_id:
                    if i < len(SESSIONS):  # Standard session
                        ...
                        # You might want to implement custom session handling here
                    break
            st.session_state.show_session_manager = False
        ...
        # Create session manager and display grid
        session_manager = SessionManager(SESSIONS, st.session_state.user_id)
        
        # Add create session button
        if st.button("➕ Create New Session", key="create_session_from_manager"):
        ...
        # Add back button
        if st.button("⬅️ Back", key="session_manager_back_btn"):
            st.session_state.show_session_manager = False
            st.rerun()
...
# Show modals if needed (in priority order)
